...
import { Terminal, Cpu, Network, Bot, ChevronDown, ArrowRight, Zap, Users, Clock, Star, CheckCircle, MessageSquare, DollarSign, ShieldCheck, Send, Mic, MicOff, X, TrendingUp } from "lucide-react";

/* ── icon map (content.json uses string names) ── */
const ICONS: Record<string, React.ComponentType<{ size?: number; color?: string }>> = {
  MessageSquare, DollarSign, Cpu, Network, Bot, Terminal, Clock, Zap, Users, Star, CheckCircle, ShieldCheck, TrendingUp
};
        ...
        {open && (
          <motion.div
            ...
            <div style={{ padding: "14px 16px", borderBottom: "1px solid rgba(16,185,129,0.1)", display: "flex", alignItems: "center", gap: 10 }}>
              <div style={{ width: 32, height: 32, borderRadius: 10, background: "linear-gradient(135deg,#10b981,#059669)", display: "grid", placeItems: "center" }}>
                <Bot size={16} color="#fff" />
              </div>
              <div>
                <div style={{ fontSize: 13, fontWeight: 600, color: "#f0fdf4" }}>AICoachLab Assistant</div>
                <div style={{ fontSize: 11, color: "rgba(167,243,208,0.5)" }}>Ask anything about AI</div>
              </div>
              {hasVoice && (
                <motion.button onClick={toggleVoice} whileTap={{ scale: 0.9 }}
                  style={{ marginLeft: "auto", background: listening ? "rgba(16,185,129,0.2)" : "transparent", border: listening ? "1px solid #10b981" : "1px solid rgba(167,243,208,0.15)", borderRadius: 8, padding: "6px 8px", cursor: "pointer", display: "flex", alignItems: "center", gap: 4 }}>
                  {listening
                    ? <motion.span animate={{ opacity: [1, 0.4, 1] }} transition={{ duration: 1, repeat: Infinity }}><Mic size={13} color="#10b981" /></motion.span>
                    : <MicOff size={13} color="rgba(167,243,208,0.5)" />}
                  <span style={{ fontSize: 10, color: listening ? "#10b981" : "rgba(167,243,208,0.5)" }}>{listening ? "Listening..." : "Voice"}</span>
                </motion.button>
              )}
            </div>
              ...
              {messages.map((m, i) => (
                <motion.div key={i} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.2 }}
                  ...
                  <div style={{
                    ...
                    background: m.role === "user" ? "rgba(16,185,129,0.2)" : "rgba(255,255,255,0.04)",
                    border: m.role === "user" ? "1px solid rgba(16,185,129,0.3)" : "1px solid rgba(167,243,208,0.08)",
                    fontSize: 13, color: "#f0fdf4", lineHeight: 1.55
                  }}>
                    {m.text}
                  </div>
                </motion.div>
              ...
              <div style={{ padding: "0 12px 8px", display: "flex", flexWrap: "wrap", gap: 6 }}>
                {config.suggestedQuestions.map((q, i) => (
                  <button key={i} onClick={() => send(q)}
                    style={{ fontSize: 11, padding: "5px 10px", borderRadius: 999, background: "rgba(16,185,129,0.07)", border: "1px solid rgba(16,185,129,0.18)", color: "rgba(167,243,208,0.8)", cursor: "pointer", textAlign: "left" }}>
                    {q}
                  </button>
                ))}
              </div>
            ...
            {/* input */}
            <div style={{ padding: "10px 12px", borderTop: "1px solid rgba(16,185,129,0.08)", display: "flex", gap: 8, alignItems: "center" }}>
              <input
                value={input} onChange={e => setInput(e.target.value)}
                onKeyDown={e => { if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); send(input); } }}
                placeholder="Ask about AI, models, costs..."
                style={{ flex: 1, background: "rgba(255,255,255,0.04)", border: "1px solid rgba(167,243,208,0.1)", borderRadius: 10, padding: "8px 12px", color: "#f0fdf4", fontSize: 13, outline: "none" }}
              />
              <motion.button onClick={() => send(input)} whileTap={{ scale: 0.9 }} disabled={!input.trim() || loading}
                style={{ width: 36, height: 36, borderRadius: 10, background: input.trim() ? "linear-gradient(135deg,#10b981,#059669)" : "rgba(167,243,208,0.07)", border: "none", cursor: input.trim() ? "pointer" : "not-allowed", display: "flex", alignItems: "center", justifyContent: "center" }}>
                <Send size={14} color={input.trim() ? "#fff" : "rgba(167,243,208,0.3)"} />
              </motion.button>
            </div>
          </motion.div>
        )}
...
function TrendingTicker({ topics }: { topics: string[] }) {
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "6px 14px", borderRadius: 999, background: "rgba(16,185,129,0.06)", border: "1px solid rgba(16,185,129,0.15)", maxWidth: "fit-content" }}>
      <TrendingUp size={11} color="#10b981" />
      <span style={{ fontSize: 11, color: "#10b981", fontWeight: 600, letterSpacing: "0.06em" }}>TRENDING</span>
      <AnimatePresence mode="wait">
        <motion.span key={idx} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -6 }} transition={{ duration: 0.3, ease }}
          style={{ fontSize: 12, color: "rgba(167,243,208,0.75)" }}>
          {topics[idx]}
        </motion.span>
      </AnimatePresence>
    </div>
...
export default function Home() {
  ...
  useEffect(() => {
    if (!content) return;
    const t = setInterval(() => setActiveTrack(p => (p + 1) % content.tracks.length), 2800);
    return () => clearInterval(t);
  }, [content]);
    ...
    <div style={{ background: "#030a06", minHeight: "100vh", fontFamily: "'Inter', sans-serif", color: "#f0fdf4", overflowX: "hidden" }}>
      ...
      <nav className="acl-nav" style={{ position: "fixed", top: 0, left: 0, right: 0, zIndex: 100, display: "flex", alignItems: "center", justifyContent: "space-between", padding: "14px 32px", background: "rgba(3,10,6,0.7)", backdropFilter: "blur(16px)", borderBottom: "1px solid rgba(16,185,129,0.1)" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <span style={{ fontWeight: 700, fontSize: 15, letterSpacing: "-0.3px" }}>AI<span style={{ color: "#10b981" }}>Coach</span>Lab</span>
        </div>
        ...
        <div className="acl-hero-grid">
          ...
          <motion.div variants={staggerContainer} initial="hidden" animate="visible" style={{ display: "flex", flexDirection: "column", gap: 20 }}>
            ...
            <motion.div variants={fadeUp} custom={1}>
              <span style={{ display: "inline-flex", alignItems: "center", gap: 6, padding: "4px 12px", borderRadius: 999, border: "1px solid rgba(16,185,129,0.3)", background: "rgba(16,185,129,0.07)", fontSize: 12, color: "#10b981", fontWeight: 500, letterSpacing: "0.04em" }}>
                <Zap size={11} /> {content.hero.eyebrow}
              </span>
            </motion.div>

            {/* headline */}
            <motion.h1 variants={fadeUp} custom={2} style={{ fontSize: "clamp(2rem,4vw,3.2rem)", fontWeight: 800, lineHeight: 1.12, letterSpacing: "-0.03em", margin: 0 }}>
              {content.hero.headline[0]}{" "}
              <span style={{ background: "linear-gradient(120deg,#10b981,#06b6d4)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>{content.hero.headline[1]}</span>
            </motion.h1>

            <motion.p variants={fadeUp} custom={3} style={{ fontSize: 16, color: "rgba(167,243,208,0.65)", lineHeight: 1.65, margin: 0, maxWidth: 440 }}>
            ...
            <motion.div variants={fadeUp} custom={6} style={{ display: "flex", gap: 20, alignItems: "center", flexWrap: "wrap" }}>
              {content.trust.map((t, i) => (
                <span key={i} style={{ fontSize: 12, color: "rgba(167,243,208,0.5)", display: "flex", alignItems: "center", gap: 4 }}>{t.icon} {t.text}</span>
              ))}
            </motion.div>
          </motion.div>
            ...
            {content.stats.map((s, i) => {
              ...
              return (
                <motion.div key={i} whileHover={{ scale: 1.04, y: -3 }} transition={{ duration: 0.2 }}
                  style={{ padding: "20px 18px", borderRadius: 14, background: "rgba(255,255,255,0.03)", border: "1px solid rgba(167,243,208,0.08)", backdropFilter: "blur(10px)", display: "flex", flexDirection: "column", gap: 8 }}>
                  ...
                  <div style={{ fontSize: 12, color: "rgba(167,243,208,0.5)", fontWeight: 500 }}>{s.label}</div>
                </motion.div>
              );
            })}
            ...
            <div style={{ gridColumn: "1 / -1", display: "flex", flexDirection: "column", gap: 8 }}>
              {content.curriculum.map((c, i) => (
                <motion.div key={i} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.4 + i * 0.07, duration: 0.4, ease }}
                  whileHover={{ x: 4 }} style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 14px", borderRadius: 10, background: activeTrack === i ? `${c.color}10` : "rgba(255,255,255,0.02)", border: activeTrack === i ? `1px solid ${c.color}40` : "1px solid rgba(167,243,208,0.07)", cursor: "pointer", transition: "background 0.35s, border 0.35s" }}
                  onClick={() => setActiveTrack(i)}>
                  <CheckCircle size={13} color={c.color} />
                  <span style={{ fontSize: 11, color: "rgba(167,243,208,0.4)", minWidth: 36 }}>{c.week}</span>
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: 13, fontWeight: 500 }}>{c.title}</div>
                    <div style={{ fontSize: 11, color: "rgba(167,243,208,0.35)", marginTop: 1 }}>{c.desc}</div>
                  </div>
        ...
        <div className="acl-pricing-grid">
          ...
          <div>
            <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, ease }}
              ...
              <h2 id="curriculum" style={{ fontSize: "clamp(1.4rem,3vw,2rem)", fontWeight: 800, letterSpacing: "-0.03em", margin: "0 0 6px" }}>
                <span style={{ background: "linear-gradient(120deg,#10b981,#06b6d4)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>Simple pricing.</span> No surprises.
              </h2>
              <p style={{ fontSize: 14, color: "rgba(167,243,208,0.5)", margin: 0 }}>Cancel monthly any time. Team invoices available.</p>
            </motion.div>
              ...
              {content.plans.map((p, i) => (
                <motion.div key={i} initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
                  ...
                  style={{ padding: "22px 18px", borderRadius: 16, position: "relative", overflow: "hidden",
                    background: p.highlight ? "rgba(16,185,129,0.08)" : "rgba(255,255,255,0.02)",
                    border: p.highlight ? "1px solid rgba(16,185,129,0.35)" : "1px solid rgba(167,243,208,0.08)",
                    backdropFilter: "blur(10px)" }}>
                  {p.badge && (
                    <span style={{ position: "absolute", top: 12, right: 12, fontSize: 10, padding: "2px 8px", borderRadius: 999, background: "rgba(16,185,129,0.2)", color: "#10b981", fontWeight: 600 }}>{p.badge}</span>
                  )}
                  <div style={{ fontSize: 12, color: "rgba(167,243,208,0.5)", fontWeight: 500, marginBottom: 8 }}>{p.name}</div>
                  <div style={{ display: "flex", alignItems: "baseline", gap: 4, marginBottom: 4 }}>
                    <span style={{ fontSize: 28, fontWeight: 800, letterSpacing: "-0.04em", color: p.highlight ? "#10b981" : "#f0fdf4" }}>{p.price}</span>
                    <span style={{ fontSize: 12, color: "rgba(167,243,208,0.4)" }}>{p.period}</span>
                  </div>
                  <div style={{ display: "flex", flexDirection: "column", gap: 6, margin: "14px 0 18px" }}>
                    {p.features.map((f, j) => (
                      <div key={j} style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 12, color: "rgba(167,243,208,0.65)" }}>
                        <CheckCircle size={11} color={p.highlight ? "#10b981" : "#059669"} /> {f}
                      </div>
                    ))}
                  </div>
          ...
          <motion.div initial={{ opacity: 0, x: 20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, ease }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {content.faqs.map((f, i) => (
                <motion.div key={i} layout style={{ borderRadius: 12, background: "rgba(255,255,255,0.02)", border: "1px solid rgba(167,243,208,0.08)", overflow: "hidden" }}>
                  <button onClick={() => setFaqOpen(faqOpen === i ? null : i)}
                    style={{ width: "100%", display: "flex", justifyContent: "space-between", alignItems: "center", padding: "14px 16px", background: "transparent", border: "none", color: "#f0fdf4", fontSize: 13, fontWeight: 500, cursor: "pointer", textAlign: "left", gap: 8 }}>
                    <span>{f.q}</span>
                    <motion.span animate={{ rotate: faqOpen === i ? 180 : 0 }} transition={{ duration: 0.25 }}>
                      <ChevronDown size={14} color="rgba(167,243,208,0.4)" />
                    </motion.span>
                  </button>
                  <AnimatePresence>
                    {faqOpen === i && (
                      <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.25, ease }}>
                        <p style={{ margin: 0, padding: "0 16px 14px", fontSize: 13, color: "rgba(167,243,208,0.55)", lineHeight: 1.6 }}>{f.a}</p>
                      </motion.div>
                    )}
                  </AnimatePresence>
                </motion.div>
            ...
            <motion.div whileInView={{ opacity: 1, y: 0 }} initial={{ opacity: 0, y: 16 }} viewport={{ once: true }} transition={{ delay: 0.3, duration: 0.5 }}
              style={{ marginTop: 24, padding: "20px", borderRadius: 14, background: "rgba(16,185,129,0.06)", border: "1px solid rgba(16,185,129,0.2)", textAlign: "center" }}>
              <p style={{ margin: "0 0 12px", fontSize: 14, fontWeight: 600 }}>Ready to use AI smarter?</p>
              <motion.a href="#" whileHover={{ scale: 1.04, boxShadow: "0 0 24px rgba(16,185,129,0.4)" }} whileTap={{ scale: 0.96 }}
                style={{ display: "inline-flex", alignItems: "center", gap: 6, padding: "10px 22px", borderRadius: 10, background: "linear-gradient(135deg,#10b981,#059669)", color: "#fff", fontWeight: 600, fontSize: 14, textDecoration: "none", cursor: "pointer" }}>
                Get started today <ArrowRight size={14} />
              </motion.a>
              <p style={{ margin: "10px 0 0", fontSize: 11, color: "rgba(167,243,208,0.4)" }}>30-day money-back guarantee · no questions asked</p>
            </motion.div>
          </motion.div>
        </div>
      </section>
      ...
      {/* ── footer ── */}
      <footer style={{ position: "relative", zIndex: 1, padding: "24px 32px", borderTop: "1px solid rgba(167,243,208,0.06)", display: "flex", justifyContent: "space-between", alignItems: "center", maxWidth: 1100, margin: "0 auto" }}>
        <span style={{ fontSize: 13, fontWeight: 700 }}>AI<span style={{ color: "#10b981" }}>Coach</span>Lab</span>
        <span style={{ fontSize: 12, color: "rgba(167,243,208,0.3)" }}>© 2026 · Use AI smarter.</span>
      </footer>
