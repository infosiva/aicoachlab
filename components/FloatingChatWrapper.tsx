    ...
    <>
      <motion.button onClick={() => setOpen(o => !o)} whileHover={{ scale: 1.08 }} whileTap={{ scale: 0.95 }}
        style={{ position: 'fixed', bottom: 24, right: 24, width: 52, height: 52, borderRadius: '50%',
          ...
          justifyContent: 'center', boxShadow: '0 4px 20px rgba(249,115,22,0.35)', zIndex: 1000, fontSize: 20 }}>
        {open ? '✕' : '🎯'}
      </motion.button>
      <AnimatePresence>
        {open && (
          <motion.div initial={{ opacity: 0, y: 10, scale: 0.97 }} animate={{ opacity: 1, y: 0, scale: 1 }}
            ...
            style={{ position: 'fixed', bottom: 88, right: 24, width: 320, height: 420,
              background: 'rgba(8,8,20,0.97)', border: '1px solid rgba(249,115,22,0.35)', borderRadius: 16,
              display: 'flex', flexDirection: 'column', zIndex: 1000, overflow: 'hidden', backdropFilter: 'blur(20px)' }}>
            <div style={{ padding: '12px 16px', borderBottom: '1px solid rgba(249,115,22,0.35)', fontSize: 13, fontWeight: 700, color: '#f8fafc' }}>
              AICoachLab AI
            </div>
            ...
            <div style={{ padding: '10px 12px', borderTop: '1px solid rgba(249,115,22,0.35)', display: 'flex', gap: 8 }}>
              <input value={input} onChange={e => setInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && send()}
                placeholder="Ask about coaching or your goals..."
                style={{ flex: 1, background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(249,115,22,0.35)',
